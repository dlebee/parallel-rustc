...
pub async fn run_build_v2(config: &BuildConfig) -> Result<BuildV2Summary, String> {
    ...
    // Pass 2 — clean the target dir so replay actually does work.
    println!("[3/4] cargo clean...");
    let clean_started = Instant::now();
    cargo_clean(config.manifest_path.as_deref()).await?;
...
async fn replay_phases(
    ...
    for (pi, phase) in phases.iter().enumerate() {
        ...
        for wave in phase.chunks(concurrency) {
            let mut set: JoinSet<Result<(), String>> = JoinSet::new();
            for &idx in wave {
